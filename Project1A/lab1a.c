...
int exit_flag = 0; //exit status of parent_process
...
void terminal_input(); //handles the read_write from the terminal for the parent
void shell_input(); //handles the read_write from the shell for the parent
void handle_sigpipe(); //handles the SIGPIPE signal
...
int main(int argc, char *argv[]) {
  ...
  while(1) {
    ...
    switch(c) {
    case 's': //--shell case
      shell_flag = 1; //turns shell flag on
      strcpy(program_name, optarg); //stores the argument in program_name
      break;
  ...
  signal(SIGPIPE, handle_sigpipe); //activates the SIGPIPE handler
  fork_ret = fork();
...
void parent_process() {
	...
	exit(1);
      ...
      if(fds[0].revents & POLLIN) { //terminal input waiting
	terminal_input();
      }
      if(fds[1].revents & POLLIN) { //shell input waiting
	shell_input();
      }
      else if(fds[1].revents & POLLERR || fds[1].revents & POLLHUP) { //error
	exit_flag = 1;
      }
    }
    close(pipefd1[1]);
    ...
    fprintf(stderr, "SHELL EXIT SIGNAL=%d STATUS=%d\n", WIFSIGNALED(status), WEXITSTATUS(status)); //program termination
    exit(0);
}
